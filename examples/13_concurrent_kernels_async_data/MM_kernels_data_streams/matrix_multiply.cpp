...
int main(int argc, char *argv[]){
    ...
    double *h_A_1, *h_B_1, *h_C_1;
    gpuCheck( hipHostMalloc(&h_A_1, bytes) );
    gpuCheck( hipHostMalloc(&h_B_1, bytes) );
    gpuCheck( hipHostMalloc(&h_C_1, bytes) );

    /* Allocate memory for host matrices 2 */
    double *h_A_2, *h_B_2, *h_C_2;
    gpuCheck( hipHostMalloc(&h_A_2, bytes) );
    gpuCheck( hipHostMalloc(&h_B_2, bytes) );
    gpuCheck( hipHostMalloc(&h_C_2, bytes) );

    /* Allocate memory for host matrices 3 */
    double *h_A_3, *h_B_3, *h_C_3;
    gpuCheck( hipHostMalloc(&h_A_3, bytes) );
    gpuCheck( hipHostMalloc(&h_B_3, bytes) );
    gpuCheck( hipHostMalloc(&h_C_3, bytes) );
    ...
    gpuCheck( hipMemcpyAsync(d_A_1, h_A_1, bytes, hipMemcpyHostToDevice, stream1) );
    gpuCheck( hipMemcpyAsync(d_B_1, h_B_1, bytes, hipMemcpyHostToDevice, stream1) );
    gpuCheck( hipMemcpyAsync(d_C_1, h_C_1, bytes, hipMemcpyHostToDevice, stream1) );

    /* Copy data from host matrices 2 to device matrices 2 */
    gpuCheck( hipMemcpyAsync(d_A_2, h_A_2, bytes, hipMemcpyHostToDevice, stream2) );
    gpuCheck( hipMemcpyAsync(d_B_2, h_B_2, bytes, hipMemcpyHostToDevice, stream2) );
    gpuCheck( hipMemcpyAsync(d_C_2, h_C_2, bytes, hipMemcpyHostToDevice, stream2) );

    /* Copy data from host matrices 3 to device matrices 3 */
    gpuCheck( hipMemcpyAsync(d_A_3, h_A_3, bytes, hipMemcpyHostToDevice, stream3) );
    gpuCheck( hipMemcpyAsync(d_B_3, h_B_3, bytes, hipMemcpyHostToDevice, stream3) );
    gpuCheck( hipMemcpyAsync(d_C_3, h_C_3, bytes, hipMemcpyHostToDevice, stream3) );
    ...
    gpuCheck( hipMemcpyAsync(h_C_1, d_C_1, bytes, hipMemcpyDeviceToHost, stream1) );

    /* Copy data from device matrix to host matrix 2 (only need result, d_C_2) */
    gpuCheck( hipMemcpyAsync(h_C_2, d_C_2, bytes, hipMemcpyDeviceToHost, stream2) );

    /* Copy data from device matrix to host matrix 3 (only need result, d_C_3) */
    gpuCheck( hipMemcpyAsync(h_C_3, d_C_3, bytes, hipMemcpyDeviceToHost, stream3) );
